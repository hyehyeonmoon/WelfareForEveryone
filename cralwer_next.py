...
def crawler_url(page_num):
    ...
    url="https://www.bokjiro.go.kr/welInfo/retrieveWelInfoBoxList.do?searchIntClId=06&welSrvTypeCd=01"
    driver.get(url)
    driver.implicitly_wait(3)
    #tqdm(range(1,page_num+1),mininterval=1)
    for page in range(1,page_num+1):
        ...
        if page==1:
            ...
            #print("page : ", page, "->", page%10+1)
            path='//*[@id="contents"]/div[4]/div/a['+str(page%10+1)+']'
            category = driver.find_element_by_xpath(path)
            category.click()
            driver.implicitly_wait(3)
            #time.sleep(1)
        ...
        try:
            ...
            print('html page does not exist :', url, "block :", block)
            continue
...
df.columns=['title', 'summary', 'who', 'criteria', 'what', 'how', 'calls', 'sites']
...
result_final=df[['who', 'criteria', 'what', 'how', 'calls', 'sites']]
query = ("""INSERT INTO welfare_detail (who,criteria, what, how,  calls, sites) VALUES (%s, %s, %s, %s, %s, %s)""") #table name 바꾸는 거 기억하기
input_data=[tuple(x) for x in result_final.to_records(index=False)]
cursor.executemany(query, input_data) #error 나는 이유가 tuple이 아니라 numpy.record이기 때문
